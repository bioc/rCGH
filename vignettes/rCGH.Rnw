...
\begin{itemize}
    ...
    \item{byGene}: the final by-gene dataset.
\end{itemize}
...
filePath <- system.file("extdata", "Affy_cytoScan.cyhd.CN5.CNCHP.txt.bz2",
    package = "rCGH")
cgh <- readAffyCytoScan(filePath, sampleName = "CytoScan-Example",
                        labName = "myLab")
@
...
{\Rfunction{plotDensity} shows how \emph{EM} has modeled the \emph{LRR}
distribution, and what peak has been chosen for centralizing the profile
(indicated in bold).}
...
\Rfunction{plotProfile()} allows the genomic profile visualization. When the
genes table is built, any gene of interest can be added to the plot by passing
a valid HUGO symbol. Other arguments can be used to color the segments
according to specified gain/loss thresholds, or to change the plot title.
\\
...
<<getProfile, fig.width=7.5, fig.height=6.5, fig.show='hide'>>=
multiplot(cgh, "erbb2")
@
...
When the profile centering doesn't seem appropriate, the \Rfunction(recenter)
function allows the user to choose another centralization value. The new choice
has to be specified as the peak number to use: peaks are indexed, from 1 to k (
from left to right) as they appear on the density plot.

\vspace{5mm}

<<recenter, fig.width=7.5, fig.height=3.5, fig.show='hide'>>=
# Recentering on the first peak with mean higher than LRR=1
recenter(cgh) <- which(getParam(cgh)$peakMeans>1)[1]
plotProfile(cgh)
@
...
center the profile, but any other peak can be chosen, using \Rfunction{recenter}
.
}
...
the \emph{Genes table} simultaneously on that chromosome.
\\
...
\begin{itemize}
    \item{Gene Symbol} : displays any existing gene, providing its official 
    HUGO symbol.
    \item{Show chromosome} : displays the entire profile (default is 'All') 
    or one specific chromosome.
    \item{Recenter profile} : recenter the profile on-the-fly. The gene 
    values  will be also updated in the 'Genes table'.
    \item{Rescale max(y)} : adjusts the top y-axis (0<y) using a proportion 
    of the maximum value.
    \item{Rescale min(y)} : adjusts the bottom y-axis (y<0) using a 
    proportion of the minimum value.
    \item{Gain threshold (Log2ratio)} : defines the gain threshold. 
    Segments higher than this value are colored in blue, and the 
    'Genes table' is filtered, consequently.
    \item{Loss threshold (Log2ratio)} : same as 'Gain threshold' for 
    losses. Segments lower than this value are colored in red.
    \item{Download - Profile} : to download the profile as it is displayed 
    on the screen, including modifications.
    \item{Download - LOH} : to download the LOH plot as it is displayed 
    on the screen, including modifications.
    \item{Download - Table} : to download the 'Genes table', including 
    modifications.
\end{itemize}
...
\incfig[h]{view.pdf}{.95\textwidth}{Interactive profile.}
{
...
simultaneously and can be exported, after modifications.
}
